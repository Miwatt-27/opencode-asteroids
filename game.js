...
class PowerUp {
  ...
  draw() {
    ...
    const color = this.type === 'shield' ? '#5cf' : '#0ff';
...
function update(dt) {
  ...
  for (const pu of powerups) {
    if (!pu.dead && dist(ship, pu) < ship.radius + pu.radius) {
      pu.dead = true;
      if (pu.type === 'shield') ship.shield = 5;
...
function drawHUD() {
  ...
  // Barras de estado (escudo y velocidad), apiladas de abajo hacia arriba
  let barY = H - 36;
  if (ship.shield > 0) {
    const shieldTime = Math.max(0, ship.shield);
    ctx.textAlign   = 'center';
    ctx.fillStyle   = '#5cf';
    ctx.font        = '13px monospace';
    ctx.fillText(`ESCUDO ${shieldTime.toFixed(1)}s`, W / 2, barY);
    ...
    ctx.fillStyle = '#5cf';
    ctx.fillRect(W / 2 - 80, barY + 6, 160 * (shieldTime / 5), 6);
    barY -= 24;
  }
  if (ship.speedBoost > 0) {
    ...
    ctx.fillStyle   = '#0ff';
    ctx.font        = '13px monospace';
    ctx.fillText(`VELOCIDAD ${boostTime.toFixed(1)}s`, W / 2, barY);
    ...
    ctx.fillStyle = '#0ff';
    ctx.fillRect(W / 2 - 80, barY + 6, 160 * (boostTime / 5), 6);
  }
}
